...
feature 'Viewing ical events' do
  let!(:recording) { FactoryGirl.create(:recording, datetime: Date.new(2013, 01, 01), state: :requested)}
  ...
  scenario do
    ...
    expect(ical_page).to have_event_status("TENTATIVE")
  end

  scenario "status confirmed" do
    recording.update(state: :confirmed)
    ical_page.visit_page
